...
res=$(mysql -s -N --user="$user" --password="$password" --database="$database" --execute="SELECT id, url FROM img_urls WHERE hash='' AND state=0 LIMIT 1")
if [ -z "$res" ]; then
...
mysql $dbargs --execute="SELECT id, url FROM img_urls WHERE hash='' AND state=0 LIMIT 200" | while read id url; do
    #check timeout to exit
    end=`date +%s`
    runtime=$((end-start))
    #echo "Runtime: "$runtime
    if [ $runtime -gt 17 ]; then
        echo "Condition"
        break
    fi
    ...
    T="$(date +%s)"
    ext="${url##*.}"
    fname="/var/tmp/mysql/all_images/${id}_${T}_temp."$ext
    echo $fname
    
    #set timeout as time left
    #CURLARGS="-o "$fname" -s --connect-timeout 3 --retry 0 --max-time "$((17-runtime))
    CURLARGS="-f -o "$fname" -o "$fname" -o "$fname" -o "$fname" -s -L --connect-timeout 3 --retry 0 --max-time "$((17-runtime))
    #download file
    ...
    if [ -e $fname ] && [ -f $fname ]; then
        ...
        then
            ...
            mysql  $dbargs --execute="UPDATE img_urls SET hash='"$md5"' WHERE id="$id
        ...
        mysql  $dbargs --execute="UPDATE img_urls SET state=1 WHERE id="$id
    fi
done

mysql $dbargs --execute="FLUSH TABLES img_urls, imagefiles"